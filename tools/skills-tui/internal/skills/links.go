...
// mirroring bash install_skill. Link failures are collected (each wrapped
// with the skill name for batch attribution) but do not stop the remaining
// links.
...
// migration). Real removal errors are collected and returned (wrapped with the
// skill name) so callers can detect a failed uninstall.
