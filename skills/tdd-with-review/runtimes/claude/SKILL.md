...
description: Compose tdd, docs, review-loop, autoreview, and local commit checkpoints into one implementation workflow. Use when the user wants a feature or bug fix implemented test-first, documented, and iterated through reviewer quality loops.
---
...
IMPORTANT: Create a local commit checkpoint between each phase so a human reviewer can observe the progression of work.
...
Create a local commit after each phase when that phase reaches its own checkpoint:
...
Each checkpoint commit should contain only the intended changes from that phase. Do not create an empty commit. Do not push checkpoint commits.
...
- Do not skip the local commit checkpoint between each phase unless the user says so explicitly.
