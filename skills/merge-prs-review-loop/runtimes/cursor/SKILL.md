...
Use the actual *review-loop* workflow for critique loops. Use native delegation only when the current surface exposes a safe mechanism for it; otherwise run the review loop inline and state that no separate reviewer was used.
