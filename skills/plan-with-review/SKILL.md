...
description: Write a detailed, TDD-based implementation plan and then iterate on the plan itself with the $review-loop skill until it meets a high quality gate. Use when the user wants a thorough, reviewed implementation plan (not the implementation) — a plan that bakes in test-first development and has been critiqued and revised before any code is written.
---
...
Honor user overrides for the plan's scope, test strategy, output location, or the loop settings — but unless the user explicitly opts out of this workflow, do not drop below `1` plan-review loop or relax the `9/10` quality gate.
...
- **A `$tdd` section** that drives the work test-first: the first failing tests or test updates to write, the expected red state, the implementation path to green, and the refactor pass. Every behavior change should be tied to a test.
- **Verification commands** and the expected evidence of success.
...
Run the `$review-loop` skill against the plan itself, with the defaults above (min `1`, max `12`, quality gate `9/10`).

Each loop:

1. Spawn a **separate reviewer subagent** (fresh context — give it the user request, the relevant repo observations, and the draft plan, not your reasoning).
...
3. Revise the plan to address Critical and Important findings, keeping a running list of prior feedback to avoid regressions.
