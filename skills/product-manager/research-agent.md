...
Standard-mode research agents return findings as structured markdown; Workflow-mode `agent()` calls return schema-validated JSON instead (see the next section).
...
## Structured Output (Workflow mode)

When Phase 2 runs in **Workflow mode**, each dimension's `agent()` call is forced to a JSON schema instead of markdown. Every dimension shares a common `findings[]` shape, plus dimension-specific fields. Claims marked `time_sensitive: true` feed the adversarial verification stage.

**Shared shape** — every dimension returns:
...
**Per-dimension fields** (added alongside `findings[]`):
