...
Use `gh` for repository, PR, review-thread, and diff operations unless the user provides another GitHub integration. Pass `--repo <owner/repo>` to `gh` commands whenever the target repository is not the current checkout.
...
Display the report as a Markdown table. Include PR repo, number, title, URL, total unresolved thread count, one row per unresolved thread, the decision, evidence, action, and URL.
