...
- `catalog/skills/` is the complete generated Codex catalog consumed by the
  Vercel skills CLI. It publishes all 10 first-party names as flat
  shared-plus-Codex assemblies and all 11 third-party names as faithful copies
  with generated per-package attribution.
...
Regenerate or check the complete catalog with:
...
Do not edit `catalog/` directly. For each first-party package, the generator
copies `shared/` to the package root and overlays `runtimes/codex/` at that
same root; the emitted root `SKILL.md` is the authored Codex entry point. No
router, `runtimes/` directory, or Claude payload is published. Third-party
packages preserve authored bytes and modes and receive a deterministic
`ATTRIBUTION.md`. Generation validates the complete effective Codex input
before atomically replacing output, and `--check` reports drift without
publishing. This path is independent of the Go installer and hooks.
...
The official Codex installation command is:

```bash
npx skills add https://github.com/brian-bell/agent-skills/tree/main/catalog --agent codex
```

The generated catalog supports Codex only. Use repeated `--skill` arguments
and optional `--global --yes --copy` flags for unattended installs. See
`docs/installing-skills.md` for listing, updating, and maintenance commands.

For Claude Code, hooks, and GitHub import, run the existing Go installer:
...
# Generated Codex catalog and isolated runtime acceptance
...
`scripts/test_generate_skills_catalog.py` verifies complete discovery, flat
Codex assembly, metadata, provenance, modes, fail-closed publication,
determinism, drift diagnostics, and checked-in output through the generator's
public interface. `scripts/test-skills-catalog-cli.sh` uses the pinned Vercel
CLI with a temporary home and cache to list and copy-install all 21 packages
into Codex only. `scripts/test-skills-catalog-codex.sh` requires a logged-in
Codex CLI and proves that an isolated install exposes UI metadata and loads the
direct root Codex instructions in an ephemeral read-only session.
`scripts/test-skills-catalog-remote.sh` backs the push-to-main pinned remote
smoke and the scheduled latest-CLI compatibility smoke in
`.github/workflows/codex-skills-catalog.yml`.
