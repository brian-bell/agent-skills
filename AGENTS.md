...
- `catalog/skills/` is the checked-in Vercel CLI canary generated by
  `scripts/generate-skills-catalog.py`. It currently publishes only
  `feature-review` and `last30days`; it is not the complete portable inventory.
...
## Vercel Catalog Canary

Regenerate the checked-in canary with:

```bash
python3 scripts/generate-skills-catalog.py
```

Do not edit `catalog/` directly. The generator emits `feature-review` as one
portable package with a root runtime router and isolated
`runtimes/{codex,claude}/` assemblies, then copies `last30days` with its
supporting files, modes, Codex metadata, and attribution. This canary path is
independent of the Go installer and hooks; it neither invokes nor replaces
them.
...
# Generated Vercel catalog canary and isolated pinned-CLI install
...
`scripts/test_generate_skills_catalog.py` verifies runtime assembly, routing,
metadata, provenance, modes, safe publication, and checked-in output through
the generator's public interface. `scripts/test-skills-catalog-cli.sh` uses
the pinned Vercel CLI with a temporary home and cache to list and copy-install
both canaries into explicit Codex and Claude Code targets.
